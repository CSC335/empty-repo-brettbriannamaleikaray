...
public class RoundPane extends GridPane {
	...
	public RoundPane(MemoryGameGUI memoryGame, Mode mode,
		...
		this.getStylesheets().addAll(
				getClass().getResource("TextOutline.css").toExternalForm());
	}
		...
		timer = new AnimationTimer() {
			...
			public void handle(long l) {
				...
				if (!remaining.isNegative()) {
					...
					RoundPane.this.memoryGame.getLoginPane().getCurrentAccount()
							.addRound(curRound);
			...
			private String format(Duration remaining) {
				return String.format("%02d:%02d:%02d", remaining.toHoursPart(),
	...
	private void layoutGUI() {
		...
		returnToTitle.setStyle(
				"-fx-background-color: papayawhip; -fx-text-fill: black;");
		this.add(returnToTitle, 4, 1);

		cacti = new ImageView(
				new Image("file:src/images/cardbacks/cardback_cacti.png", 125,
						175, false, false));
		cities = new ImageView(
				new Image("file:src/images/cardbacks/cardback_cities.png", 125,
						175, false, false));
		mammals = new ImageView(
				new Image("file:src/images/cardbacks/cardback_mammals.png", 125,
						175, false, false));
		mountains = new ImageView(
				new Image("file:src/images/cardbacks/cardback_mountains.png",
						125, 175, false, false));
		reptiles = new ImageView(
				new Image("file:src/images/cardbacks/cardback_reptiles.png",
						125, 175, false, false));
		...
		// cacti chosen
		cacti.addEventHandler(MouseEvent.MOUSE_CLICKED,
				new EventHandler<MouseEvent>() {

					/**
					 * Handler for if the cacti card back is clicked. Starts a
					 * new round with the cacti deck and sets the cardBackPath
					 * to the cacti images. Then calls a private method to
					 * create a full deck of cacti cards.
					 * 
		...
		// cities chosen
		cities.addEventHandler(MouseEvent.MOUSE_CLICKED,
				new EventHandler<MouseEvent>() {

					/**
					 * Handler for if the cities card back is clicked. Starts a
					 * new round with the cities deck and sets the cardBackPath
					 * to the city images. Then calls a private method to create
					 * a full deck of city cards.
					 * 
		...
		// mammals chosen
		mammals.addEventHandler(MouseEvent.MOUSE_CLICKED,
				new EventHandler<MouseEvent>() {

					/**
					 * Handler for if the mammals card back is clicked. Starts a
					 * new round with the mammals deck and sets the cardBackPath
					 * to the mammal images. Then calls a private method to
					 * create a full deck of mammals cards.
					 * 
		...
		// mountains chosen
		mountains.addEventHandler(MouseEvent.MOUSE_CLICKED,
				new EventHandler<MouseEvent>() {

					/**
					 * Handler for if the mountains card back is clicked. Starts
					 * a new round with the mountains deck and sets the
					 * cardBackPath to the mountain images. Then calls a private
					 * method to create a full deck of mountains cards.
					 * 
		...
		// reptiles chosen
		reptiles.addEventHandler(MouseEvent.MOUSE_CLICKED,
				new EventHandler<MouseEvent>() {

					/**
					 * Handler for if the reptiles card back is clicked. Starts
					 * a new round with the reptiles deck and sets the
					 * cardBackPath to the reptile images. Then calls a private
					 * method to create a full deck of reptiles cards.
					 * 
	...
	private void makeFullDeck() {
		...
		for (int i = 0; i < 16; i += 1) {
			textLabels[i] = getCardTextLabel(curRound.getCard(i).getName(), 125,
					175);
			cardImages.add(new ImageView(
					new Image(cardBackPath, 125, 175, false, false)));
	...
	private void flipCardOne() {
		...
		if (clickedCardOne.isFlipped() == false) {
			...
			cardImages.get(cardOneIndex)
					.setImage(new Image(
							curRound.getCard(cardOneIndex).getFileName(), 125,
							175, false, false));
			...
			cardImages.get(cardOneIndex)
					.setImage(new Image(cardBackPath, 125, 175, false, false));
	...
	private void flipCardTwo() {
		...
		if (clickedCardTwo.isFlipped() == false) {
			...
			cardImages.get(cardTwoIndex)
					.setImage(new Image(
							curRound.getCard(cardTwoIndex).getFileName(), 125,
							175, false, false));
	...
	private Card getClickedCard(MouseEvent event) {
		...
		final int cardGap = 10; // gap between cards, same horizontal or
								// vertical
		...
		for (int i = 0; i < 16; i += 1) {
			if (curRound.getCard(i).isDestroyed() == false) {
				if (clickX >= (topLeftX + (i * (cardWidth + cardGap)))
						- (((i / 4) * rowWidth))
						&& clickX <= (topLeftX + (i * (cardWidth + cardGap)))
								- (((i / 4) * rowWidth)) + cardWidth
						&& clickY >= topLeftY
								+ ((i / 4) * (cardHeight + cardGap))
						&& clickY <= topLeftY
								+ ((i / 4) * (cardHeight + cardGap)
										+ cardHeight)) {
					clickCol = i % 4;
	...
	private void checkCards() {
		if (curRound.isActive()) {
			...
			if (!clickedCardOne.equals(nullCard)
				...
				if (clickedCardOne.getName().equals(clickedCardTwo.getName())) {
					...
					// Set the card's image to be a transparent placeholder
					// image
					cardImages.get(cardOneIndex)
							.setImage(new Image("file:src/images/emptycard.png",
									125, 175, false, false));
					cardImages.get(cardTwoIndex)
							.setImage(new Image("file:src/images/emptycard.png",
									125, 175, false, false));
					...
					// Flip cards back over, reset click variables, remove text
					// labels
					flipCardOne();
					flipCardTwo();
		...
		if (!curRound.isActive()) {
			...
			this.memoryGame.getLoginPane().getCurrentAccount()
					.addRound(curRound);
	 ...
	 */
	private Label getCardTextLabel(String cardName, double cardWidth,
			double cardHeight) {
		// Dimensions of card image files
		final double CARD_IMAGE_WIDTH = 202;
		final double CARD_IMAGE_HEIGHT = 275;

		// Dimensions cards will be drawn at (if you change this you'll need to
		// change
		...
		cardText.setPadding(
				new Insets(0, 0, drawnCardHeight * 0.15 * yScale, 0));
