...
public class RoundPaneWithTextAndBG extends Application {
	...
	private void makeFullDeck() {
		...
		for(int i = 0; i < 16; i += 1)
			textLabels[i] = getCardTextLabel(deck.getCard(i).getName(), 150, 200);
	}
	...
	private void setMouseHandler() {
		pane.setOnMouseClicked(event -> {
			...
			if (firstGuess) {
				...
				pane.add(new ImageView(new Image(clickedCardOne.getFileName(), 150, 200, false, false)), colOne,
						rowOne);
				pane.add(textLabels[(rowOne-1) * 4 + colOne], colOne, rowOne);
			} else {
				firstGuess = true;
				try {
					Thread.sleep(1000);
		...
		pane.setOnMousePressed(event -> {
			...
			if(firstGuess == false) {
				...
				pane.add(textLabels[(rowTwo-1) * 4 + colTwo], colTwo, rowTwo);
			}
		});
